Shader "ShirayuriMeshibe/URP/ParticleMeshAudience_Atras_CommonMotion (ParticleSystem)"
{ 
    Properties
    {
        ...
        [Header(UniqueAnimation)]
        _TotalFrame_U("Frame count", Float) = 2000
        _SequenceFrame("Anime Count", Float) = 120
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct MyParticleInstanceData
            {
                float3x4 transform;
                uint color;
                float4 motiondata;      //前のシーケンスディレイ、次のシーケンスディレイ、ペンライトの色彩(hue)、ペンライトの明るさ(value)
                float4 motiondata2;   //前のシーケンスのoffset_t、次のシーケンスのoffset_t
            };
            ...
            struct appdata
            {
                ...
                float4 motiondata : TEXCOORD3;
                float4 motiondata2 : TEXCOORD4;
#endif
